...
export default function DistributorProfile() {
  ...
  useEffect(() => {
    const fetchDistributorProfile = async () => {
      ...
      try {
        ...
        if (response.data.status === "success" && response.data.data?.distributor) {
          const distributorData: DistributorProfile = response.data.data.distributor;
          ...
          if (distributorData.DocumentsURL) {
            setKycDocuments(prev => prev.map(doc => ({
              ...doc,
              uploaded: true,
              url: distributorData.DocumentsURL
            })));
          }
        } else {
          toast({
  ...
  const infoSections = [
    ...
    },
    {
      ...
      icon: Wallet,
      items: [
        { label: "Master Distributor ID", value: userInfo.masterDistributorId },
        { 
