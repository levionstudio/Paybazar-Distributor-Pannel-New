...
const DistributorFundRetailer = () => {
  ...
  useEffect(() => {
    const checkAuth = () => {
      ...
      try {
        ...
        // Verify user role is distributor
        if (decoded.user_role !== "distributor") {
          toast({
            title: "Access Denied",
            description: "This page is only accessible to distributors.",
            variant: "destructive",
          });
          window.location.href = "/login";
          return;
  ...
  useEffect(() => {
    ...
    const fetchRetailers = async () => {
      ...
      try {
        const endpoint = `${import.meta.env.VITE_API_BASE_URL}/retailer/get/distributor/${tokenData.user_id}`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // IMPORTANT: Check distributor's wallet balance (not retailer's)
    // Distributor is PAYING from their own wallet to fund the retailer
    ...
    // Payload for fund transfer from distributor to retailer
    const payload = {
      from_id: tokenData.user_id,        // Distributor's user_id
    ...
    console.log("Distributor Balance:", walletBalance);
    ...
    try {
      ...
      // Refresh distributor's wallet balance
      try {
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription className="mt-1 text-white/90">
                    Select retailer to transfer funds
                  </CardDescription>
                </div>
              </div>
            </CardHeader>
